...
| **HV battery state of charge, energy remaining, range, distance since charge, temperature, charger power** | **proven**: six supervised enhanced reads (UDS service `22`) all answer from the Battery System Manager. They cross-check each other — range ÷ state of charge gives 333 mi at full against a 329 mi EPA rating, and the charger reads exactly 0 kW while the pack is measurably discharging. State of charge is coarser and slower than it looks: it changed three times in a 2900-second session while `energy_kwh` took 59 distinct values, in jumps of −0.502, −0.500 and +0.400 at irregular gaps. Neither the step nor the interval is fixed, so a short charge can report zero gained however much energy went in — use `energy_kwh`, which moves continuously. Dashboard cross-check still outstanding — see [GM enhanced candidates](docs/GM_ENHANCED_CANDIDATES.md) |
...
level from `energy_kwh` (not `soc_pct`, which jumps a few tenths of a percent at irregular intervals — the only three changes in one 2900-second session were −0.502, −0.500 and +0.400, at gaps of 143 s and 2688 s — so its three printed decimals are an offset carried between jumps rather than resolution) and a warmth
