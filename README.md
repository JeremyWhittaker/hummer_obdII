...
| **HV battery state of charge, energy remaining, range, distance since charge, temperature, charger power** | **proven**: six supervised enhanced reads (UDS service `22`) all answer from the Battery System Manager. They cross-check each other — range ÷ state of charge gives 333 mi at full against a 329 mi EPA rating, and the charger reads exactly 0 kW while the pack is measurably discharging. State of charge is coarser than it looks: it moves in 0.500% steps, so a charge under about 0.96 kWh reports zero gained no matter how much energy went in. Dashboard cross-check still outstanding — see [GM enhanced candidates](docs/GM_ENHANCED_CANDIDATES.md) |
...
level from `energy_kwh` (not `soc_pct`, which moves in measured steps of **0.500%** — every transition in a 227-row session was 0.500 or 0.502, so its three printed decimals are an offset carried between steps rather than resolution) and a warmth
