...
| **C** Controlled physical correlation | **Half done** | The correlation engine exists (`hummer-obd-decode`, which reports per-field spans and refuses to quote a correlation without one). What does **not** exist is a labelled-experiment schema — no way to record "this session was a cold soak at 4 °C with HVAC off". That is a real gap |
