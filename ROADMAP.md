...
| **C** Controlled physical correlation | **Built; needs a human at the vehicle** | `hummer-obd-experiment` records what a person observed beside each session, `mark` timestamps an intervention as it happens, and `hummer-obd-respond` reports which columns moved between marked states — ambient temperature, dashboard SoC, what the charger's own display said. Its load-bearing field is `label_source`: a label *derived from the session CSV* does not break the circle of correlating the truck's numbers against its other numbers, and the schema refuses to let an inferred label carry an outside measurement. 23 sidecars exist and **every one is marked inferred**, because nobody was standing at the vehicle with a thermometer |
| **D** Charge-session study | **Tooling complete, needs a charge** | `analyze.py` already produces a charge report rather than a meaningless distance-and-efficiency one, and the recorder captures everything relevant every cycle. What is missing is a *labelled* charge with independent truth values written down beside it. Needs a charge event |
