...
CONFIDENCE: Final[dict[str, Evidence]] = {
    ...
    "4127": Evidence(2, ("40",), ("parked", "driving", "charging"),
                     ...
                     "word. " + ABA + "It holds ONE value per phase: 234 "
                     "across all 99 cold-soak samples and all 80 of the first "
                     "A/C phase, exactly 601 across all 44 heat samples, then "
                     "234 again, stepping within one poll of each switch the "
                     "owner threw. The 2026-09-04 drive extends this: it held "
                     "246 bit-exact across 138 consecutive samples spanning "
                     "0-143 kph, -275 to +630 A and 346 kW of HV power swing. "
                     "246 appears in no other session and is first seen in the "
                     "exact poll of first wheel motion -- the sample before "
                     "has all four wheel speeds at 0 and reads 234, the sample "
                     "at 16:07:44 has all four at 1 kph and reads 246. "
                     "CORRECTION: an earlier version of this entry stated that "
                     "value 1048 'occurs in 410 samples of which every one has "
                     "negative pack current -- it appears only while "
                     "charging'. THAT IS FALSE and it was published from a "
                     "MEAN. The summary table showed mean pack current -8.6 A "
                     "for the 1048 group and that was written up as a claim "
                     "about every sample; the script written to check it "
                     "crashed, and the claim shipped anyway. Re-derived: of "
                     "443 samples at 1048 carrying a current, 235 are "
                     "negative, 184 are strictly POSITIVE and 24 are zero, and "
                     "all 184 positives have charger_5401_raw = '00', i.e. the "
                     "charger inactive. The rule that does hold without "
                     "exception is different and better: of 477 samples at "
                     "1048, ZERO carry any speed_kph value at all. Charging is "
                     "merely a subset of the no-wheel-speed state, which is "
                     "why the charging story looked right on parked data. "
                     "Today's drive supplied the disambiguating case the "
                     "parked corpus could not: the field steps to 1048 six "
                     "seconds after the last wheel-speed report, parked, "
                     "unplugged, SoC flat and pack current positive -- and "
                     "0x4124 drops 1000 -> 0 in the same poll. So 234, 246 and "
                     "1048 track whether the powertrain is reporting road "
                     "speed, not what temperature anything is. No scaling is "
                     "claimed. It is 2 and not 3 because 246 rests on a single "
                     "session and 601 on a single heat cycle"),
    ...
    "F191": Evidence(0, (), ("probe only",),
                     ...
                     "7F 22 31 everywhere asked"),
    "2429": Evidence(2, ("17",), ("parked", "driving", "charging"),
                     "the source calls this nominal (rated) pack voltage, /64. "
                     "That is wrong, and the 2026-09-04 drive replaces it with "
                     "a positive reading rather than only a refutation. "
                     "NOT A VOLTAGE, by a test that does not depend on "
                     "correlation: in drive-20260904T020049Z.csv the measured "
                     "pack voltage swings 0.83 V -> 377.89 V across 526 "
                     "samples as the contactors work, and this field reads "
                     "0x5806 in every one of those 526. A field that does not "
                     "move while its claimed quantity moves 377 V is not that "
                     "quantity. It also moves the wrong way under load: at "
                     "peak draw the real pack voltage SAGS to 377.20 V while "
                     "this field /64 RISES to 400.00 V, r = -0.554. "
                     "WHAT IT IS: a signed, zero-referenced drive/regen "
                     "quantity. 22534 (0x5806) is the zero point -- constant "
                     "across 1083 stationary samples corpus-wide, and the only "
                     "value present in 7 of the 8 sessions that carry it. It "
                     "ignores HVAC completely: 308 of 309 parked A-B-A samples "
                     "read 0x5806 through cold soak, A/C, heat and A/C, and "
                     "the one exception is the row where wheel speed first "
                     "reads 3 kph. It REVERSES about that zero with torque "
                     "direction: all 25 drive samples below -20 A sit under "
                     "22534 and 73 of 75 above +20 A sit over it, matching "
                     "sign on 25 of 27 zero crossings. That reversal is what "
                     "rules out elapsed time -- r against elapsed_s is -0.113 "
                     "over 1017 s. "
                     "It is NOT a function of pack current: the gain falls "
                     "from 18.2 counts/A at 30 kph to 3.9 at 132 kph, so the "
                     "previously published '~16.4 counts per amp' was an "
                     "artefact of pooling speeds. It tracks electrical power "
                     "divided by speed, R2 = 0.926 over the 82 samples above "
                     "40 kph with a near-zero intercept. Read that claim "
                     "carefully: hv_power_kw is pack_v x pack_a and pack_v "
                     "varies only +/-2.4%, so P/v is very nearly current over "
                     "speed, and calling it TRACTIVE FORCE imports an "
                     "assumption of roughly constant drivetrain efficiency. "
                     "One genuinely independent check supports the force "
                     "reading, using module 28 only and no pack current at "
                     "all: predicting the field from longitudinal_g and speed "
                     "squared gives R2 = 0.766 across all 140 samples and an "
                     "implied vehicle mass of 3415 kg, the right order for "
                     "this vehicle. "
                     "NO EQUATION IS SHIPPED. The constant is 2.30-2.42 "
                     "newtons per count, which is not a divisor a designer "
                     "picks; it becomes round only by assuming an unmeasured "
                     "rolling radius or final-drive ratio. The zero point is "
                     "not round under any candidate scaling either. Must be "
                     "read as one big-endian u16, not byte 0 alone: the low "
                     "byte is pinned at 0x06 in all 320 baseline samples, "
